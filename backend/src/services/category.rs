...
pub async fn get_all_categories_with_subs(pool: &PgPool, family_id: Uuid) -> Result<Vec<CategoryWithSubs>, AppError> {
    let categories = sqlx::query_as::<_, Category>(
        "SELECT * FROM categories WHERE user_id IS NULL OR family_id = $1 ORDER BY type, sort_order, name",
    )
    .bind(family_id)
    .fetch_all(pool)
    .await?;
    ...
    let subcategories = sqlx::query_as::<_, Subcategory>(
        "SELECT s.* FROM subcategories s
         ...
         WHERE (c.user_id IS NULL OR c.family_id = $1)
           AND (s.user_id IS NULL OR s.family_id = $1)
...
pub async fn get_category(
    ...
    let cat = sqlx::query_as::<_, Category>(
        "SELECT * FROM categories WHERE id = $1 AND (user_id IS NULL OR family_id = $2)",
    )
    .bind(category_id)
    .bind(family_id)
    .fetch_optional(pool)
...
pub async fn update_category(
    ...
    if existing.family_id != Some(family_id) {
        return Err(AppError::Forbidden(
...
pub async fn get_subcategories(
    ...
    let subs = sqlx::query_as::<_, Subcategory>(
        "SELECT * FROM subcategories
         WHERE category_id = $1 AND (user_id IS NULL OR family_id = $2)
