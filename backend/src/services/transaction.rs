...
fn classify_transaction(csv_cat: &str, note: &str, txn_type: &str) -> (&'static str, Option<&'static str>) {
    ...
    // 通讯网络（话费/宽带/会员订阅）—— 放在数码电子之前优先匹配
    if csv_cat == "通讯网络"
        || has_keyword(&text, &[
            "话费", "充话费", "流量充值", "中国移动", "中国联通", "中国电信",
            "宽带", "网费", "网络费", "光纤", "上网费",
            "会员", "vip", "视频会员", "音乐会员", "订阅", "爱奇艺", "优酷",
            "腾讯视频", "芒果tv", "哔哩哔哩", "b站", "网飞", "netflix",
            "spotify", "网盘", "云盘", "百度网盘",
        ])
    {
        let sub = if has_keyword(&text, &[
            "话费", "充话费", "流量充值", "中国移动", "中国联通", "中国电信",
        ]) {
            "手机话费"
        } else if has_keyword(&text, &["宽带", "网费", "网络费", "光纤", "上网费"]) {
            "宽带网络"
        } else {
            "会员订阅"
        };
        return ("通讯网络", Some(sub));
    }
